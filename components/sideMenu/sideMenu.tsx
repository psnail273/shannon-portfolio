...
export default function SideMenu({ isSideMenuOpen, handleHamburgerMenuClose }: { isSideMenuOpen: boolean, handleHamburgerMenuClose: () => void }) {
  return (
    ...
    >
      <div className='flex flex-row items-center justify-end min-h-[150px] pr-[110px]'>
      ...
      <ul className='flex flex-col gap-3 justify-start'>
        { pages.map((page) => (
          <Link href={ page.href } className='group relative' key={ page.name } onClick={ handleHamburgerMenuClose }>
            ...
            <li className='hover:text-[#1238e0] transition-colors duration-200 transition-ease-in-out px-[140px]'>{ page.name }</li>
          </Link>
        )) }
      </ul>
      <div className="fixed flex items-center justify-start bottom-0 left-0 right-0 h-[140px] px-[140px] bg-white">
        <input 
          type="text"
